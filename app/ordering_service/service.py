...
async def create_user_order(
    ...
    for date_item in item.dates:
        dates.append(
            await date_api.create_date(db, DatesCreate(**date_item.dict(exclude_unset=True), order_id=order_db.id)))
    for comment_item in item.comments:
        comments.append(
            await comment_api.create_comment(
                db, CommentCreate(**comment_item.dict(exclude_unset=True), author_id=customer_id, order_id=order_db.id)
            )
        )
    return OrderFullDB(**order_db.dict(), dates=dates, comments=comments)
...
async def create_order_comment(db: AsyncSession, author_id: UUID, order_id: UUID, item: CommentCreate) -> CommentDB:
    comment_item = CommentCreate(**item.dict(exclude_unset=True), author_id=author_id, order_id=order_id)
