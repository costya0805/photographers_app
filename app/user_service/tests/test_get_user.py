...
async def test_get_photographer(db_session: AsyncSession, user: PhotographerCreate, photographer: Photographer):
    user = await create_photographer(db_session, user)
    gotten_user = await get_photographer(db_session, user.id)
...
async def test_get_customer(db_session: AsyncSession, user: CustomerCreate, customer: Customer):
    user = await create_customer(db_session, user)
    gotten_user = await get_customer(db_session, user.id)
...
async def test_get_admin(user: AdminCreate, admin: Admin, db_session: AsyncSession):
    user = await create_admin(db_session, user)
    gotten_user = await get_admin(db_session, user.id)
